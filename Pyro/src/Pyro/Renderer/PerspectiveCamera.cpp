...
namespace Pyro
{
	PerspectiveCamera::PerspectiveCamera(float fovy, float aspect, float zNear, float zFar)
		: m_ProjectionMatrix(glm::perspective(glm::radians(fovy), aspect, zNear, zFar)), m_ViewMatrix(1.0f), yaw(0.0f), pitch(0.0f), m_ViewProjectionMatrix(m_ProjectionMatrix*m_ViewMatrix)
	{
